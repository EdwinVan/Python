# 爬虫抓取京东网搜索词为‘python’时前50页的商品数据{商品名、价格}，以csv格式保存到本地文件
# 2020-12-09
# fyj
...
def goods_info_first(n):
    ...
    # 书名
    name = book_info.xpath("//li/div/div[3]/a/em")
    name_2 = book_info.xpath("//li/div/div/div[2]/div[1]/div[3]/a/em")

    # # 此段代码用于测试
    # # 打印输出获取的数据
    # print("第{}页 前30条数据".format(n))
    ...
    # 将数据保存到csv本地文件
    with open('jd_Book.csv', 'a', newline='', encoding='gb18030') as f:  # 不加newline=''的话csv中两条相邻数据间会空出一行
        write = csv.writer(f)
        for i in range(len(price)):
            write.writerow([name[i].xpath('string(.)'), price[i]])
        for i in range(len(price_2)):
            write.writerow([name_2[i].xpath('string(.)'), price_2[i]])
...
def goods_info_end(n):
    ...
    # 第n页的动态加载地址
    # 格式：网址+数+&s数+&scrolling=y&log_id=UNIX时间戳
    a = time.time()  # 获取UNIX时间戳
    b = '%.4f' % a   # 保留到小数点后4位
...
def main():
    web_page_num = eval(input("请输入你想要获取的商品页数: "))
    print("*******************")
    print("程序运行,开始抓取网站数据")
    start = time.time()
    with open('jd_Book.csv', 'a', newline='', encoding='gb18030') as f:  # 不加newline=''的话csv中两条相邻数据间会空出一行
    ...
    for n in range(1, web_page_num+1):
        goods_info_first(n)
        goods_info_end(n)
    end = time.time()
    print("程序结束运行，用时{:.2f}s".format(end-start))
    print("抓取了{}个页面的商品数据".format(web_page_num))
    print("*******************")
...
# 京东页面网址
# https://search.jd.com/Search?keyword=python&wq=python&page=1&s=-4&click=0   第1页
# https://search.jd.com/Search?keyword=python&wq=python&page=3&s=56&click=0   第2页
# https://search.jd.com/Search?keyword=python&wq=python&page=5&s=116&click=0  第3页
# https://search.jd.com/Search?keyword=python&wq=python&page=7&s=176&click=0  第4页

# 京东动态加载地址
# https://search.jd.com/s_new.php?keyword=python&wq=python&page=2&s=28&scrolling=y&log_id=时间戳   第1页
# https://search.jd.com/s_new.php?keyword=python&wq=python&page=4&s=88&scrolling=y&log_id=时间戳   第2页
...
# 评论
# /html/body/div[6]/div[2]/div[2]/div[1]/div/div[2]/ul/li[1]/div/div[4]/strong/a
# /html/body/div[6]/div[2]/div[2]/div[1]/div/div[2]/ul/li[55]/div/div[4]/strong/a   零条评论
# /html/body/div[6]/div[2]/div[2]/div[1]/div/div[2]/ul/li[3]/div/div/div[2]/div[1]/div[5]/strong/a
# /html/body/div[6]/div[2]/div[2]/div[1]/div/div[2]/ul/li[9]/div/div/div[2]/div[1]/div[5]/strong/a
